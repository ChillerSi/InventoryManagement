...
class VectorService {
  ...
  List<Double> embed(byte[] bytes, String contentType) {
    ...
    node.get("vector").forEach(v -> out.add(v.asDouble()));
    return out;
  }
  ...
  List<Map<String, Object>> search(UUID tenantId, List<Double> vector, int top) {
    ...
    n.get("result")
        .forEach(
            x ->
                best.merge(
                    x.get("payload").get("productId").asText(),
                    x.get("score").asDouble(),
                    Math::max));
    return best.entrySet().stream()
        .sorted(Map.Entry.<String, Double>comparingByValue().reversed())
        .limit(top)
        .map(e -> Map.<String, Object>of("productId", e.getKey(), "similarity", e.getValue()))
        .toList();
  }
}
